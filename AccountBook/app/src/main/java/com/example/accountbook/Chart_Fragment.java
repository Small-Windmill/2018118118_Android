...
public class Chart_Fragment extends Fragment {
    int salary = 0,bonus = 0,jianzhi=0;
    int food = 0,clothes = 0,life=0,others = 0;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container,@Nullable Bundle savedInstanceState){
        ...
        pieChart.setDescriptionTextSize(50);//设置描述的字体大小
        ...
        pieChart.setTransparentCircleRadius(50f);//设置圆环的半径值

        // enable rotation of the chart by touch
        pieChart.setRotationEnabled(true);//设置饼状图是否可以旋转(默认为true)
        pieChart.setRotationAngle(25);//设置饼状图旋转的角度
        ...
        if (salary !=0)
            pieEntries.add(new PieEntry(salary,"工资"));
        if (bonus != 0)
            pieEntries.add(new PieEntry(bonus, "奖金"));
        if (jianzhi != 0)
        ...
        if (others != 0)
            pieEntries.add(new PieEntry(others, "其他"));
